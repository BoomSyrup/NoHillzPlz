...
function plotElevation( segments_length, route_index , routeslength, elevations, status) {
  ...
  if( global_counter ==routeslength){//judging
    ...
    for(i = 1; i < routeslength; i ++){
      ...
      map_replace0 = x[0].getMap();
      dir0 = x[0].getDirections();
      x[0].setMap(null);
      x[0] = null;
     x[0] = new google.maps.DirectionsRenderer({ polylineOptions: { strokeColor: "Red" } }); //draw route
     x[0].setMap(map_replace0);
     x[0].setDirections(dir0);
     x[0].setRouteIndex(0);
